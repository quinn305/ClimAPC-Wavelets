...
# Edited by Quinn Cunningham
# most of wavelet and wave_signif have been rewritten to fit a more functional framework by Quinn Cunningham

def wavelet(Y, dt, pad=0, dj=-1, s0=-1, J1=-1, mother=-1, param=-1, freq=None):
    def set_default_value(value, default):
        return value if value != -1 else default

    n1 = len(Y)
    s0 = set_default_value(s0, 2 * dt)
    dj = set_default_value(dj, 1. / 4.)
    J1 = set_default_value(J1, np.fix((np.log(n1 * dt / s0) / np.log(2)) / dj))
    mother = set_default_value(mother, 'MORLET')
    ...
    def calculate_fourier_factor(mother, param):
        mother_upper = mother.upper()
        if mother_upper == 'MORLET':
            param = 6. if param == -1 else param
            return 4 * np.pi / (param + np.sqrt(2 + param**2))
        elif mother_upper == 'PAUL':
            param = 4. if param == -1 else param
            return 4 * np.pi / (2 * param + 1)
        elif mother_upper == 'DOG':
            param = 2. if param == -1 else param
            return 2 * np.pi * np.sqrt(2. / (2 * param + 1))
        else:
            return np.nan

    def calculate_scale_period_wave(mother, param, J1, s0, dj, freq, n):
        fourier_factor = calculate_fourier_factor(mother, param)
    ...
    wave, period, scale = calculate_scale_period_wave(mother, param, J1, s0, dj, freq, n)
...
def wave_signif(Y, dt, scale, sigtest=0, lag1=0.0, siglvl=0.95,
    ...
    def get_empirical_params(mother, param):
        if mother == 'MORLET':
            if param is None:
                param = 6.
            k0 = param
            return [2., -1, -1, -1], param, (4 * np.pi) / (k0 + np.sqrt(2 + k0 ** 2))
        elif mother == 'PAUL':
            if param is None:
                param = 4
            m = param
            return [2, -1, -1, -1], param, (4 * np.pi) / (2 * m + 1)
        elif mother == 'DOG':
            if param is None:
                param = 2.
            elif param == 6:
                return [1., -1, -1, -1], param, 2 * np.pi * np.sqrt(2. / (2 * param + 1))
            m = param
            return [1., -1, -1, -1], param, 2 * np.pi * np.sqrt(2. / (2 * m + 1))
        else:
            print('Mother must be one of MORLET, PAUL, DOG')
            return None, None, None

    variance = get_variance(Y)
    empirical_params, param, fourier_factor = get_empirical_params(mother, param)
    ...
    def calculate_fft_theor(variance, lag1, freq, gws=None):
    if gws is not None:
        return gws
    else:
    ...
    def calculate_signif(fft_theor, siglvl, dof, sigtest, dofmin, chisquare_inv):
        signif = fft_theor
        if dof is None:
            dof = dofmin
    ...
    variance = get_variance(Y)
    fft_theor = calculate_fft_theor(variance, lag1, freq, gws)
    signif = calculate_signif(fft_theor, siglvl, dof, sigtest, dofmin, chisquare_inv)
